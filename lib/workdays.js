...
// Start date + a duration counted in work days. The start date is always day 1
// of the job — whatever the caller passed in is exactly what gets returned as
// day 1, even on the rare occasion it lands on a weekend/holiday/rain day —
// because the caller's start_date is also what gets stored on the job, and
// silently advancing past it here without changing that stored value used to
// leave the two disagreeing: a job could show start_date Sep 21 while its
// actual work started (and its work days were counted from) Sep 22, inflating
// the calendar span for a fixed work-day duration. Every day *after* day 1
// still only counts if isWorkDay says so. e.g. a 5-day job dropped on Monday
// ends Friday; a 1-day job dropped on any date — work day or not — ends that
// same date.
